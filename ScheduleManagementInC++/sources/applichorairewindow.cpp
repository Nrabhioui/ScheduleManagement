...
void ApplicHoraireWindow::on_actionEnregistrer_triggered()
{
    ...
    // TO DO (Etape 10)
    string aEnregister;
    aEnregister = dialogInputText("Enregistrer un horaire","Quelle horaire souhaiter vous enregistrer ?");
    Timetable::getInstance().save(aEnregister);
}   
...
void ApplicHoraireWindow::updateAllTables()
{
    /*MAJ Group*/
    clearTableGroups();
    auto group = Timetable::getInstance().getGroups();
    ...
    auto professor = Timetable::getInstance().getProfessors();
 
    for (auto it = professor.cbegin(); it != professor.cend(); ++it)
    {
        addTupleTableProfessors(it->tuple());
    }
    ...
    auto local = Timetable::getInstance().getClassrooms();
